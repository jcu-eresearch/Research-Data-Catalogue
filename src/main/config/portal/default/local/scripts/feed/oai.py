###JCU: this version of oai.py contains fixes to ensure the interface works correctly with ANDS
###In the next upgrade it can probably be removed.
###Function getPayloadContent() was modified to remove the XML declaration from each RIF-CS as it was sent out.
###The oai.py interface already returns an xml declaration, adding more for each rifcs caused errors with ANDS
    ...
    def getPayloadContent(self, payload):
        ...
        try:
            ...
            while line is not None:
                if firstLine:
                    if p.match(line.strip()) is not None:
                        self.log.debug("Ignoring xml declaration")
                        line = reader.readLine()
                    else:
                        self.log.error("First line didn't match xml declaration: ")
                        self.log.error(line)
